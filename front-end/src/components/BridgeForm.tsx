...
export const BridgeForm = ({ setShowSpinner }: Props) => {
    ...
    const [connectedMetaMaskAddress, setConnectedMetaMaskAddress] =
        useState("");
    ...
    const onClickPetraConnect = useCallback(async () => {
        ...
        try {
            ...
            // const account =
            await client.getAccount(result.address);
    ...
    const sendAptosToPeaqTransaction = useCallback(async () => {
        // public entry fun transfer_from (userAccount:&signer,userAddressPeaq:address, amount:u64)
        const transaction = {
            type: "entry_function_payload",
            function: `${process.env.APTOS_SMART_CONTRACT}::message::transfer_from`,
            arguments: [
    ...
    }, [amount, connectedMetaMaskAddress, setShowSpinner]);

    const sendPeaqToAptosTransaction = useCallback(async () => {
        try {
            setShowSpinner(true);
            await peaqSolidityContract?.methods
                .transfer_from(+amount)
                .send({ from: connectedMetaMaskAddress });
        } catch (error) {
        } finally {
            setShowSpinner(false);
        }
    ...
    }, [isAptosToPeaq, sendAptosToPeaqTransaction, sendPeaqToAptosTransaction]);
    ...
    const setupPeaqContract = useCallback(
        async (_isPetraInstalled: boolean) => {
